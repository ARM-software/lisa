...
which generated by the combination of two triggers
...
WINDOW_SIZE = 0.0001
"""A control config for fliter events. Some analyses
...
def csum(series, window=None, filter_gaps=False):
    ...
    :param filter_gaps: If set, a process being switched out
        for :mod:`trappy.stats.SchedConf.WINDOW_SIZE` is
...
def filter_small_gaps(series):
    ...
    in residency series < :mod:`trappy.stats.SchedConf.WINDOW_SIZE`
...
def residency_sum(series, window=None):
    ...
    intervals between a :mod:`trappy.stats.SchedConf.SCHED_SWITCH_OUT`
    and :mod:`trappy.stats.SchedConf.SCHED_SWITCH_IN` to track
...
def last_time(series, value, window=None):
    ...
    - If no such index is found
      :mod:`trappy.stats.SchedConf.TIME_INVAL`
      is returned
