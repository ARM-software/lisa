...
use nom::{
    ...
    character::complete::{alpha1, alphanumeric1, anychar, char, u64 as dec_u64},
    combinator::{all_consuming, cut, fail, map, map_res, not, opt, recognize, success},
...
use crate::{
    cinterp::{
        Bitmap, CompileEnv, CompileError, EmptyEnv, EvalEnv, EvalError, new_dyn_evaluator, Evaluator,
        InterpError, Value,
    },
    grammar::{grammar, Span},
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum ExtensionMacroCallType {
    Type(Type),
    AsExpr(Box<Expr>),
}

impl ExtensionMacroCallType {
    pub fn typ<'ce, CE>(&self, cenv: &CE, abi: &Abi) -> Result<Type, CompileError>
    where
        CE: CompileEnv<'ce>,
    {
        match self {
            ExtensionMacroCallType::Type(typ) => Ok(typ.clone()),
            ExtensionMacroCallType::AsExpr(expr) => expr.typ(cenv, abi),
        }
    }
}
...
pub struct ExtensionMacroCallCompiler {
    pub ret_typ: ExtensionMacroCallType,
...
type FunctionLikeExtensionMacroParser = Box<
    dyn for<'a> Fn(
            ...
            NomError<CParseError, nom::error::VerboseError<Span<'a, CGrammar>>>,
        > + Send
        + Sync,
>;
...
pub enum CParseError {
    ...
    #[error(
        "Invalid integer constant: the value does not fit in the range of any of the allowed types"
    )]
    InvalidIntegerConstant,

    #[error("Character value is out of range, only 8 bit values are supported")]
    CharOutOfRange,

    #[error("Only the REC identifier is recognized as a variable, everything other identifier is assumed to be an enumeration constant")]
    InvalidVariableIdentifier,

    #[error("Could not guess the type of the expression")]
    CouldNotGuessType(Expr),

    #[error("Could not interpret expression: {0}")]
    InterpError(Box<InterpError>),

    #[error("String encoding prefix syntax is not supported")]
    UnsupportedEncodingPrefix,

    #[error("Object-like macro {0:?} cannot be called since it is not a function-like macro")]
    CannotCallObjectLikeMacro(Identifier),

    #[error("Could not recognize input as C: {0}")]
    ParseError(VerboseParseError),
}

impl From<InterpError> for CParseError {
    fn from(err: InterpError) -> Self {
        CParseError::InterpError(Box::new(err))
    }
}
...
fn eval_unsigned(expr: Expr, abi: &Abi) -> Result<FileSize, InterpError> {
    expr.eval_const(abi, |res| {
        ...
        match x {
            ...
            _ => Err(InterpError::EvalError(Box::new(EvalError::IllegalType))),
        }
    })
}

fn resolve_extension_macro(name: &str, abi: &Abi) -> Result<ExtensionMacroKind, CParseError> {
    let abi = abi.clone();

    fn print_symbolic<const EXACT_MATCH: bool>(
        abi: Abi,
    ) -> Result<ExtensionMacroKind, CParseError> {
                    ...
                    |(val, _, delim, flags)| {
                        let flags = flags
                            ...
                            .filter_map(|(mask, flag)| -> Option<Result<_, _>> {
                                || -> Result<Option<_>, _> {
                                    let mask: u64 = mask.eval_const(&abi, |x| match x? {
                                        Value::U64Scalar(x) => Ok(x),
                                        Value::I64Scalar(x) => Ok(x as u64),
                                        _expr => Err(InterpError::EvalError(Box::new(
                                            EvalError::IllegalType,
                                        ))),
                                    })?;

                                    let flag: Option<String> = flag.eval_const(&abi, |s| {
                                        match s? {
                                            ...
                                            // code.
                                            Value::U64Scalar(0) | Value::I64Scalar(0) => Ok(None),
                                            s => match s.to_str() {
                        ...
                        let compiler = Arc::new(move |cenv: &dyn CompileEnv<'_>, abi: &_| {
                            let cval = val.clone().compile(&cenv, abi)?;
                            ...
                            > = if EXACT_MATCH {
                                ...
                                let cdelim = delim.clone().compile(&cenv, abi)?;
                                Box::new(move |env| {
                                    Ok(cdelim
                                        .eval(env)?
                                        .deref_ptr(env)?
                        ...
                        Ok(ExtensionMacroCallCompiler {
                            ret_typ: ExtensionMacroCallType::Type(Type::Pointer(Box::new(
                                abi.char_typ(),
                            ))),
                            compiler,
                        })
            ...
            parser: Box::new(move |input| {
                map_res_cut(
                    ...
                    |(val, _, array_size)| {
                        let compiler = Arc::new(move |cenv: &dyn CompileEnv<'_>, abi: &_| {
                            let cval = val.clone().compile(&cenv, abi)?;
                            let carray_size = array_size.clone().compile(&cenv, abi)?;
    ...
    match name {
        ...
        "__builtin_constant_p" => Ok(ExtensionMacroKind::FunctionLike {
            parser: Box::new(move |input| {
                map(many0(anychar), |_| ExtensionMacroCallCompiler {
                    ret_typ: ExtensionMacroCallType::Type(Type::I32),
                    compiler: Arc::new(move |_, _| {
        ...
        "__print_ns_to_secs" => Ok(ExtensionMacroKind::FunctionLike {
            parser: Box::new(move |input| {
                map_res_cut(CGrammar::assignment_expr(), |expr| {
                    Ok(ExtensionMacroCallCompiler {
                        ret_typ: ExtensionMacroCallType::Type(Type::U64),
                        compiler: Arc::new(move |cenv: &dyn CompileEnv, abi: &_| {
                            let cexpr = expr.clone().compile(&cenv, abi)?;

                            let eval = new_dyn_evaluator(move |env: &_| match cexpr.eval(env)? {
                                Value::U64Scalar(x) => Ok(Value::U64Scalar(x / 1_000_000_000)),
                                Value::I64Scalar(x) => Ok(Value::I64Scalar(x / 1_000_000_000)),
                                _ => Err(EvalError::IllegalType),
                            });
                            Ok(eval)
                        }),
        ...
        "__print_ns_without_secs" => Ok(ExtensionMacroKind::FunctionLike {
            parser: Box::new(move |input| {
                map_res_cut(CGrammar::assignment_expr(), |expr| {
                    Ok(ExtensionMacroCallCompiler {
                        ret_typ: ExtensionMacroCallType::Type(Type::U32),
                        compiler: Arc::new(move |cenv: &dyn CompileEnv, abi: &_| {
                            let cexpr = expr.clone().compile(&cenv, abi)?;
        ...
        "__get_sockaddr" | "__get_sockaddr_rel" => Ok(ExtensionMacroKind::FunctionLike {
            parser: Box::new(move |input| {
                map_res_cut(CGrammar::identifier(), |field| {
                    Ok(ExtensionMacroCallCompiler {
                        ret_typ: ExtensionMacroCallType::AsExpr(Box::new(Expr::record_field(
                            field.clone(),
                        ))),
                        compiler: Arc::new(move |cenv: &dyn CompileEnv, abi: &_| {
                            let cexpr = Expr::record_field(field.clone()).compile(&cenv, abi)?;
        ...
        "__get_dynamic_array" | "__get_rel_dynamic_array" | "__get_str" | "__get_rel_str" => {
            Ok(ExtensionMacroKind::FunctionLike {
                parser: Box::new(move |input| {
                    map_res_cut(CGrammar::identifier(), |field| {
                        Ok(ExtensionMacroCallCompiler {
                            // The type of the array is the same as REC->field. We
                            // don't have access to a compilation environment here
                            // that would let us resolve that type immediately, but
                            // we can simply defer that to a later stage when all
                            // types are known.
                            ret_typ: ExtensionMacroCallType::AsExpr(Box::new(Expr::record_field(
                                field.clone(),
                            ))),
                            compiler: Arc::new(move |cenv: &dyn CompileEnv, abi: &_| {
                                ...
                                Expr::record_field(field.clone()).compile(&cenv, abi)
                            }),
                        })
                    })
                    .parse(input)
        ...
        "__get_dynamic_array_len" | "__get_rel_dynamic_array_len" => {
            Ok(ExtensionMacroKind::FunctionLike {
                parser: Box::new(move |input| {
                    map_res_cut(CGrammar::identifier(), |field| {
                        Ok(ExtensionMacroCallCompiler {
                            ret_typ: ExtensionMacroCallType::Type(abi.long_typ()),
                            compiler: Arc::new(move |cenv: &dyn CompileEnv, abi: &_| {
                                let expr = Expr::record_field(field.clone()).compile(&cenv, abi)?;
        ...
        "__get_bitmask" | "__get_rel_bitmask" | "__get_cpumask" | "__get_rel_cpumask" => {
            Ok(ExtensionMacroKind::FunctionLike {
                parser: Box::new(move |input| {
                    map_res_cut(CGrammar::identifier(), |field| {
                        Ok(ExtensionMacroCallCompiler {
                            ret_typ: ExtensionMacroCallType::Type(abi.char_typ()),
                            compiler: Arc::new(move |cenv: &dyn CompileEnv, abi: &_| {
                                let bitmap =
                                    Expr::record_field(field.clone()).compile(&cenv, abi)?;
                                let abi = abi.clone();
                                Ok(new_dyn_evaluator(move |env: &_| {
                                    macro_rules! to_string {
                                        ($bitmap:expr) => {{
                                            let writer = OwnedScratchBox::new_in(
                                                move |out: &mut dyn fmt::Write| {
                                                    write!(out, "{}", $bitmap)
                                                        .expect("Could not render bitmap to string")
                                                },
                                                env.scratch(),
                                            );
                                    ...
                                    match bitmap {
                                        ...
                                        {
                                            let abi = abi.clone();
                                            to_string!(Bitmap::from_bytes(cast_slice(&arr), &abi))
                                        }
                                        Value::U64Array(arr)
        ...
        "__print_flags" | "__print_flags_u64" => print_symbolic::<false>(abi),
        "__print_symbolic" | "__print_symbolic_u64" => print_symbolic::<true>(abi),

        "__print_hex" => print_array_hex(abi, " "),
        "__print_hex_str" => print_array_hex(abi, ""),
        "__print_array" => {
            Ok(ExtensionMacroKind::FunctionLike {
                parser: Box::new(move |input| {
                    map_res_cut(
                        ...
                        |(val, _, array_size, _, item_size)| {
                            let compiler = Arc::new(move |cenv: &dyn CompileEnv<'_>, abi: &_| {
                                let cval = val.clone().compile(&cenv, abi)?;
                                let carray_size = array_size.clone().compile(&cenv, abi)?;
                                let citem_size = item_size.clone().compile(&cenv, abi)?;
                                ...
                                let eval = new_dyn_evaluator(move |env: &_| {
                                        ...
                                        ($item_ty:ty, $arr:expr) => {{
                                            ...
                                            let writer = OwnedScratchBox::new_in(
                                                move |out: &mut dyn fmt::Write| {
                                                    ...
                                                    closure()
                                                        .expect("could not render array to string")
                                                },
                                                env.scratch(),
                                            );

                                            (item_size, Ok(Value::Str(Str::new_procedural(writer))))
                                        }};
                                    }
                                    ...
                                    let (real_item_size, printed) = match cval.eval(env)? {
                                        ...
                                        Value::Bitmap(bitmap) => match &bitmap.chunk_size {
                                            LongSize::Bits32 => Ok(print_array!(
                                                ...
                                                bitmap.into_iter().as_chunks().map(|x| x
                                                    .try_into()
                                                    .expect("Chunk requires more than 32 bits"))
                                            )),
                                        ...
                                        Value::Str(s) => Ok(print_array!(u8, s.bytes().chain([0]))),
                                    ...
                                    if real_item_size == item_size {
                                        ...
                                        Err(PrintError::WrongItemSize(item_size, real_item_size)
                                            .into())
                                    }
                                });
                                Ok(eval)
        ...
        "__print_hex_dump" => {
            Ok(ExtensionMacroKind::FunctionLike {
                ...
                |(
                    ...
                    let compiler = Arc::new(move |cenv: &dyn CompileEnv<'_>, abi: &Abi| {
                        let endianness = abi.endianness;

                        let cprefix_str = prefix_str.clone().compile(&cenv, abi)?;
                        let cprefix_type = prefix_type.clone().compile(&cenv, abi)?;
                        let crow_size = row_size.clone().compile(&cenv, abi)?;
                        ...
                        // normally.
                        let cbuf = buf.clone().compile(&cenv, abi)?;
                        let clength = length.clone().compile(&cenv, abi)?;
                        let cascii = ascii.clone().compile(&cenv, abi)?;
                        ...
                        let eval = new_dyn_evaluator(move |env: &_| {
                            ...
                            match cbuf.eval(env)? {
                                Value::Bitmap(bitmap) => {
                                    match &bitmap.chunk_size {
                                        LongSize::Bits32 => print_array!(u32, bitmap.into_iter().as_chunks().map(|x| x.try_into().expect("Chunk requires more than 32 bits"))),
                                        LongSize::Bits64 => print_array!(u64, bitmap.into_iter().as_chunks()),
                                    }
                                }
                                Value::Raw(_, arr) => print_array!(u8, arr.iter().copied()),
                                Value::Str(s) => print_array!(u8, s.bytes().chain([0])),

                                Value::U8Array(arr) => print_array!(u8, arr.iter().copied()),
                                Value::I8Array(arr) => print_array!(i8, arr.iter().copied()),

                                Value::U16Array(arr) => print_array!(u16, arr.iter().copied()),
                                Value::I16Array(arr) => print_array!(i16, arr.iter().copied()),

                                Value::U32Array(arr) => print_array!(u32, arr.iter().copied()),
                                Value::I32Array(arr) => print_array!(i32, arr.iter().copied()),

                                Value::U64Array(arr) => print_array!(u64, arr.iter().copied()),
                                Value::I64Array(arr) => print_array!(i64, arr.iter().copied()),
        ...
        _ => Err(CParseError::InvalidVariableIdentifier),
    }
}
            ...
            alt((
                ...
                preceded(
                    ...
                    map_res_cut(hex_u64, |x| {
                        x.try_into().map_err(|_| CParseError::CharOutOfRange)
                    }),
                ),
    ...
    E: FromExternalError<I, CParseError> + nom::error::ContextError<I> + nom::error::ParseError<I>,
{
    many1(map_res_cut(
        ...
        |(prefix, string)| match prefix {
            Some(_) => Err(CParseError::UnsupportedEncodingPrefix),
            None => Ok(string),
        },
    ))
    .map(|seqs: Vec<String>| {
                ...
                alt((
                    ...
                    context("func call expr",
                        move |input| {
                            ...
                            let abi = &Self::get_ctx(&input).abi;
                            let f = f.simplify(&EmptyEnv::new(), abi);
                            match f {
                                        ...
                                        lexeme(|input: Span<'i, Self>| {
                                            match &desc.kind {
                                                ExtensionMacroKind::FunctionLike{parser} => {
                                                    ...
                                                    let mut parser = move |input| {
                                                        let (input, args) = Self::balanced_paren().parse(input)?;
                                                        let saved_args = (*args.deref()).to_vec();
                                                        let (_, compiler) = all_consuming(parser.deref()).parse(args)?;

                                                        Ok((input, Expr::ExtensionMacroCall(
                                                            ExtensionMacroCall {
                                                                args: saved_args,
                                                                desc: Arc::clone(&desc),
                                                                compiler,
                                                    ...
                                                    match parser.parse(input.clone()).finish() {
                                                        Err(err) => {
                                                            let err = match err.data {
                                                                None => FromParseError::from_parse_error(input.clone(), &err.inner),
                                                                Some(err) => err,
                                                            };
                                                            failure(input, err)
                                                        }
                                                        Ok((input, call)) => Ok((input, call)),
                                                    }
                                                }
                                ...
                                _ => {
                                    ...
                                    Ok((input, Expr::FuncCall(Box::new(f), args)))
                                }
                            }
                        }
                    ),
            ...
            lexeme(
                alt((
                    map_res_cut(
                        ...
                        |(ctx, expr)| {
                            expr.typ( &EmptyEnv::new(), ctx.abi).map_err(|_| CParseError::CouldNotGuessType(expr))
                        }
                    ),
                    tuple((
                        Self::declaration_specifier(),
                        ...
                        macro_rules! guess_typ {
                            ($x:ident, $($typ:ident),*) => {
                                match $x {
                                    ...
                                    _ => Err(CParseError::InvalidIntegerConstant),
                                }
                            }
                        }
                ...
                alt((
                    ...
                    map_res(
                        ...
                        |(ctx, id)| match id.deref() {
                            ...
                            _ => {
                                let abi = &ctx.abi;
                                let kind = resolve_extension_macro(&id, abi)?;
...
mod tests {
    ...
    fn expr_test() {
        ...
        let extension_compiler = Arc::new(closure!(
            ...
            |_, _| panic!("non-implemented compiler")
        ));
                        ...
                        desc: Arc::new(
                            ExtensionMacroDesc {
                                ...
                                kind: ExtensionMacroKind::FunctionLike {
                                    parser: Box::new(|_| panic!("non implemented parser"))
                                },
                            }
                        ),
                        compiler: ExtensionMacroCallCompiler {
                            ret_typ: ExtensionMacroCallType::Type(Type::Pointer(Box::new(Type::U8))),
    ...
    fn declaration_test() {
        ...
        test(
            ...
            Type::Array(
                ...
                ArrayKind::Fixed(Err(Box::new(InterpError::CompileError(Box::new(
                    CompileError::CannotHandleExpr(Expr::EnumConstant(
                        Type::I32,
                        "static_bar".into(),
                    )),
                ))))),
        ...
        test(
            ...
            Type::Array(
                Box::new(u64_typ.clone()),
                ArrayKind::Fixed(Err(Box::new(InterpError::CompileError(Box::new(
                    CompileError::CannotHandleExpr(Expr::EnumConstant(Type::I32, "A".into())),
                ))))),
            ),
        );
