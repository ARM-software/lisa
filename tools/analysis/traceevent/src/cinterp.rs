use core::{fmt, ops::Deref, num::Wrapping};
use std::sync::Arc;
        ...
        (Some(lhs_info), Some(rhs_info)) => Ok({
            if lhs == rhs {
                ...
                if utyp_info.rank >= styp_info.rank {
                    utyp.clone()
                } else if styp_info.width - 1 >= utyp_info.width {
                    styp.clone()
                } else {
                    styp_info.unsigned
                }
            ...
            Ternary(_, lhs, rhs) => {
                ...
                fn is_null_ptr_cst(expr: &Expr) -> bool {
                    match expr {
                        ...
                        Expr::Cast(Pointer(pointee), expr) if pointee.deref() == &Void => is_null_ptr_cst(expr),
                        _ => false,
                    }
                }
                ...
                match usual_arith_conv(lhstyp.clone(), rhstyp.clone()) {
                    ...
                    Err(_) => match (&lhstyp, &rhstyp) {
                        ...
                        (Pointer(inner), Pointer(_)) | (Pointer(_), Pointer(inner)) if inner.deref() == &Void => Ok(Pointer(Box::new(Void))),
                        (Pointer(lhstyp_), Pointer(rhstyp_))
                            if lhstyp_.resolve_wrapper() == rhstyp_.resolve_wrapper() =>
                        {
                            Ok(Pointer(Box::new(lhstyp)))
                        }
                        _ => Err(CompileError::MismatchingOperandType(self.clone(), lhstyp, rhstyp)),
                    },
                }
            }
        ...
        CE: CompileEnv<'ce>,
    {
        ...
        macro_rules! binop {
            ($ctor:expr, $lhs:expr, $rhs:expr, $op:expr) => {{
                ...
                let lhstyp = lhs.typ(cenv, abi)?;
                let rhstyp = rhs.typ(cenv, abi)?;
                let ctor: Option<fn(Box<Expr>, Box<Expr>) -> Expr> = $ctor;

                match (lhstyp, rhstyp, ctor) {
                    // Binary operation between two arithmetic types
                    (lhs, rhs, _) if lhs.is_arith() && rhs.is_arith() => {
                    ...
                    // Pointer arithmetic
                    (Type::Pointer(_lhstyp), rhstyp, Some(ctor)) if rhstyp.is_arith() => {
                    ...
                    (lhstyp, Type::Pointer(_rhstyp), Some(ctor)) if lhstyp.is_arith() => {
                        recurse(
                            Cast(
...
mod tests {
        ...
        fn field_getter(&self, id: &str) -> Result<Box<dyn Evaluator>, CompileError> {
            match id {
                "runtime_u32_field" => {
                    ...
                    Ok(new_dyn_evaluator(move |_env| match *stage.lock().unwrap() {
                ...
                "runtime_u32_ptr" | "runtime_void_ptr" | "runtime_char_ptr" => Ok(new_dyn_evaluator(|_| {
                    Ok(Value::U64Scalar(42))
                })),
                _ => Err(CompileError::UnknownField),
            }
        }
    ...
    fn interp_test() {
        ...
        test(b"42 + ((-1) * 4)", signed(42 - 4 * 1));
        test(b"((  signed long)42) + ((-1) * 4)",   signed(42 - 4 * 1));
        test(b"((unsigned long)42) - 4", unsigned(42 - 4 * 1));
        test(b"((unsigned long)42) + ((unsigned long)-4)", unsigned(42 - 4 * 1));
        test(b"((unsigned long)42) + (0-4)", unsigned(42 - 4 * 1));
        test(b"((unsigned long)42) + (-4)", unsigned(42 - 4 * 1));
        test(b"((unsigned long)42) + ((-1) * 4)", unsigned(42 - 4 * 1));

        // Pointer arithmetic
        test(b"(unsigned int*)(((unsigned long)(unsigned int*)42) -4)", unsigned(42 - 4 * 1));
        test(b"(unsigned int*)(((unsigned long)(unsigned int*)42) + (-4))", unsigned(42 - 4 * 1));
        test(b"((unsigned long)(unsigned int*)42) + ((-1) * 4)", unsigned(42 - 4 * 1));
        test(b"(unsigned int*)(((unsigned long)(unsigned int*)42) + ((-1) * 4))", unsigned(42 - 4 * 1));

        test(b"REC->runtime_u32_ptr", unsigned(42));
        test(b"REC->runtime_u32_ptr + 1", unsigned(42 + 4 * 1));
        test(b"REC->runtime_u32_ptr - 1", unsigned(42 - 4 * 1));
        test(b"REC->runtime_u32_ptr + (-1)", unsigned(42 - 4 * 1));
        test(b"1 + REC->runtime_u32_ptr", unsigned(42 + 4 * 1));
