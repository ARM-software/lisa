                                    ...
                                    ($arr:expr) => {{
                                        let writer = OwnedScratchBox::new_in(
                                            move |out: &mut dyn fmt::Write| {
                                                ...
                                                closure().expect(
                                                    "could not render hex array to string",
                                                )
                                            },
                                            env.scratch(),
                                        );
                    ...
                    |(val, _, delim, flags)| {
                        let flags = flags
                            ...
                            .filter_map(|(mask, flag)| -> Option<Result<_, _>> {
                                || -> Result<Option<_>, _> {
                                    let mask: u64 =
                                        mask.eval_const(penv.abi(), |x| match x? {
                                            Value::U64Scalar(x) => Ok(x),
                                            Value::I64Scalar(x) => Ok(x as u64),
                            ...
                            let eval = new_dyn_evaluator(move |env: &_| {
                                ...
                                let writer = OwnedScratchBox::new_in(
                                    move |out: &mut dyn fmt::Write| {
                                        ...
                                        closure().expect(
                                            "could not render symbolic values to string",
                                        )
                                    },
                                    env.scratch(),
                                );
