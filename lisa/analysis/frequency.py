...
###############################################################################
    ...
    def plot_cluster_frequencies(self, title='Clusters Frequencies'):
        ...
        axes.set_ylim(
                (self._trace.plat_info['freqs']['big'][0] - 100000)/1e3,
                (self._trace.plat_info['freqs']['big'][-1] + 100000)/1e3
        )
        if len(bfreq) > 0:
        ...
        axes.set_ylim(
                (self._trace.plat_info['freqs']['little'][0] - 100000)/1e3,
                (self._trace.plat_info['freqs']['little'][-1] + 100000)/1e3
        )
        if len(lfreq) > 0:
    ...
    def plot_cpu_frequencies(self, cpus=None):
        ...
        for plot_idx, cpu_id in enumerate(freq):
            ...
            if 'clusters' in self._trace.plat_info:
                for cluster,cpus in self._trace.plat_info['clusters'].items():
                    if cpu_id not in cpus:
                        continue
                    freqs = self._trace.plat_info['freqs'][cluster]
                    break
            else:
    ...
    def plot_cluster_frequency_transitions(self, clusters=None, pct=False):
        ...
        for c in _clusters:
            ...
            cpu_id = self._trace.plat_info['clusters'][c.lower()][0]
            t = self.df_cpu_frequency_transitions(cpu_id)
