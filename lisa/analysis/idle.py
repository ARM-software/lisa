...
from lisa.datautils import series_integrate, series_adjacent_index
...
###############################################################################
    ...
    def _idle_cpus_at(self, timestamp, active_signal_fn):
        ...
        for cpu in range(self.trace.cpus_count):
            ...
            # and vice-versa.
            prev_idx = series_adjacent_index(series_active, timestamp, method="prev")

            if prev_idx is None:
                continue

            if not series_active.iloc[prev_idx]:
                idles.append(cpu)

        return sorted(idles)

    @signal_cpu_active.used_events
    def idle_cpus_at(self, timestamp, as_sched=False):
        """
        :returns: list of CPUs that were idle at the given ``timestamp``
        """
        return self._idle_cpus_at(
            timestamp,
            self.signal_cpu_sched_active if as_sched else self.signal_cpu_active)
