...
// DEPRECATED: use lisa__sched_pelt_cfs_tg instead
...
// DEPRECATED: Use lisa__sched_pelt_cfs_task and lisa__sched_pelt_cfs_tg instead
...
#if HAS_KERNEL_FEATURE(SE_PELT)
TRACE_EVENT(lisa__sched_pelt_cfs_task,

	TP_PROTO(int cpu, int pid, const char *comm, const struct sched_avg *avg),

	TP_ARGS(cpu, pid, comm, avg),
	...
	TP_printk(
		"cpu=%d comm=%s pid=%d load=%lu util=%lu update_time=%llu"
#if HAS_KERNEL_FEATURE(SCHED_AVG_RBL)
		" " RBL_LOAD_STR "=%lu"
#endif
		,
...
#if HAS_KERNEL_FEATURE(SE_PELT)
TRACE_EVENT(lisa__sched_pelt_cfs_tg,
...
// DEPRECATED: use lisa__sched_util_est_cfs_task or lisa__sched_util_est_cfs_tg instead
...
#if HAS_KERNEL_FEATURE(SE_UTIL_EST)
TRACE_EVENT(lisa__sched_util_est_cfs_task,
	...
	TP_printk("cpu=%d comm=%s pid=%d enqueued=%u ewma=%u util=%lu",
		__entry->cpu, __entry->comm, __entry->pid,
		__entry->enqueued, __entry->ewma, __entry->util)
	);
#endif
...
#if HAS_KERNEL_FEATURE(SE_UTIL_EST)
TRACE_EVENT(lisa__sched_util_est_cfs_tg,
...
// DEPRECATED: Use lisa__sched_util_est_cfs_tg instead
...
TRACE_EVENT(lisa__uclamp_cfs_task,

	TP_PROTO(const struct task_struct *p, const struct rq* rq),
...
TRACE_EVENT(lisa__uclamp_rq,

	TP_PROTO(const struct rq *rq),
	...
	TP_printk("cpu=%d uclamp_min=%lu uclamp_max=%lu",
		__entry->cpu, __entry->uclamp_min, __entry->uclamp_max
	)
);
#endif
