...
class VideoPreview(Workload):
    """
    Android VideoPreview workload
    """
    ...
    def __init__(self, test_env):
        super(VideoPreview, self).__init__(test_env)
        self._log = logging.getLogger('VideoPreview')
        self._log.debug('Workload created')

    def run(self, out_dir, duration_s=30, collect='systrace'):
        """
        Run a video preview workload
        ...
        """
        self._log.info("Running VideoPreview for {}s and collecting {}".format(duration_s, collect))
