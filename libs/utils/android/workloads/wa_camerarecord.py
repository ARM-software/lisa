...
from android import Wlauto, Workload
...
class WaCameraRecord(Wlauto):
    """
    Camera Record workload
    """
    ...
    workload_name = 'camerarecord'


    def __init__(self, test_env):
        super(WaCameraRecord, self).__init__(test_env)
        self._log = logging.getLogger('Camera Record')
    ...
    def run(self, exp_dir, agenda, collect=''):
        ...
        nrg_report = Wlauto.wa_run(exp_dir, _agenda,
                                   WaCameraRecord.workload_name, collect=collect)

        # Return frame stats file
    ...
    def _is_available(target):
        if Wlauto._wload_is_available(WaCameraRecord.workload_name):
            return True
        return False
