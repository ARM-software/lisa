...
a73_cluster_active_states = OrderedDict([
...
a73_cluster_idle_states = OrderedDict([
    ("WFI",               102),
    ("cpu-sleep-0",       102),
    ("cluster-sleep-0",   0),
])

a73_cpu_active_states = OrderedDict([
...
a73_cpu_idle_states = OrderedDict([
    ("WFI",               18),
    ("cpu-sleep-0",       0),
    ("cluster-sleep-0",   0),
])
...
def a73_cpu_node(cpu):
    return EnergyModelNode(cpu=cpu,
                           active_states=a73_cpu_active_states,
                           idle_states=a73_cpu_idle_states)
...
hikey960_energy = EnergyModel(
    root_node=EnergyModelRoot(
        children=[
            ...
            EnergyModelNode(
                name="cluster_a73",
                active_states=a73_cluster_active_states,
                idle_states=a73_cluster_idle_states,
                children=[a73_cpu_node(c) for c in a73s])]),
