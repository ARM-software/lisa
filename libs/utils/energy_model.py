...
###########################################
# Sanity Checks helpers for Energy Models #
###########################################
...
def _get_powers_list(node):
    ...
    """
    return [state.power for state in node.active_states.values()]
...
def _get_capacities_list(node):
    ...
    """
    return [state.capacity for state in node.active_states.values()]
...
def check_active_states_nb(energy_model, freqs_nb):
    ...
    succeed = True
    failed_nodes = []
    for i, fd in enumerate(energy_model.freq_domains):
        for cpu in fd:
            cpu_node = energy_model.cpu_nodes[cpu]
            active_state_nb = len(cpu_node.active_states.keys())
            if active_state_nb != freqs_nb[i]:
                failed_nodes.append(cpu_node.name)

        # Check higher levels of the EM
        node = energy_model.cpu_nodes[fd[0]]
        while node.parent.active_states:
            node = node.parent
            active_state_nb = len(node.active_states.keys())
            if active_state_nb != freqs_nb[i]:
...
def are_placements_equal(energy_model, expected, placement):
    """
...
def get_expected_placement(energy_model, group_util):
    """
...
def ideal_placements(energy_model):
    ...
    for i in range(nb_groups):
        ...
        exp_cap = [0] * nb_groups
        exp_cap[i] = avg_cap[i]
        expected = get_expected_placement(energy_model, exp_cap)

        # Compare the expected placement and the obtained one
        if not are_placements_equal(energy_model, expected, placement):
