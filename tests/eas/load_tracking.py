...
class FreqInvarianceTest(LisaTest):
    ...
    known period and duty cycle to estimate an expected mean value for util_avg
    for each frequency. The util_avg value is extracted from scheduler trace
    ...
    def get_expected_util_avg(self, experiment):
        ...
        mean_run = self.get_task_activations(experiment).mean()['length']

        return UTIL_SCALE * (mean_run / task_period) * freq_scaling_factor
